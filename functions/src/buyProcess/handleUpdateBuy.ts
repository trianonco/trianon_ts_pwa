...
export default function(request: any, response: any): Promise<any> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      orderData.meta['createdAt'] = getDateString(
        orderData.meta.createdAt.toDate()
      );
      orderData.meta['updatedAt'] = getDateString(
        orderData.meta.updatedAt.toDate()
      );
...
function get_HTML_InVoice(order: any) {
  return new Promise((resolve, reject) => {
    ...
    bucket
      .file('Misc/customerInVoice.html')
      ...
      .catch((error: any) => {
        ...
        reject({
          ...
          resume:
            'Error downloading html template using google storage service',
          error: error
        });
      });
  });
...
function get_PDF_InVoice(html: any, id: string) {
  return new Promise((resolve: any, reject: any) => {
    ...
    pdf
      .create(html, options)
      .toFile(localPDFFile, function(err: any, res: any) {
        if (err) {
          reject({
            target: 'handleUpdateBuy.ts',
        ...
        bucket
          .upload(localPDFFile, {
            destination: 'Shop-InVoices/' + id + '.pdf',
          ...
          .catch((error: any) => {
            reject({
              ...
              method: 'get_PDF_InVoice',
              resume:
                'Error Uploading PDF file to bucket storage google storage service',
              error: error
            });
          });
      });
  ...
  return new Promise((resolve, reject) => {
    const TEMPLATE_VARIABLES = {
      ...
      ORDER_PRODUCT_PICTURE:
        ...
        '%2Fhd%2F' +
        `${order.products[0].ref_photo_code}-01.jpg` +
        '?' +
        `alt=media&token=18c776df-f5a7-45a0-9012-16f780381d92`,
